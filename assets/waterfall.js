		var waterfallCanvas = function(c, cw, ch, m_color){
			...
			this.Particle.prototype.render = function(){			
				_this.ctx.strokeStyle = m_color;//'hsla('+this.hue+', '+this.saturation+'%, '+this.lightness+'%, .05)';
				_this.ctx.beginPath();
				_this.ctx.moveTo(this.x, this.y);
				_this.ctx.lineTo(this.x, this.y + this.height);
				_this.ctx.lineWidth = this.width;
				_this.ctx.lineCap = 'round';
				_this.ctx.stroke();
			};
			
			this.Particle.prototype.renderBubble = function(){				
				_this.ctx.fillStyle = m_color;//'hsla('+this.hue+', 40%, 40%, 1)';
				_this.ctx.fillStyle = m_color;//'hsla('+this.hue+', '+this.saturation+'%, '+this.lightness+'%, .3)';
				_this.ctx.beginPath();
